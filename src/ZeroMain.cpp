...
bool CreateOverlayWindow()
{
    ...
    // Implement the Black Stage: Create a Solid Black Win32 Window (HWND) covering the full monitor resolution.
    g_Hwnd = CreateWindowExW(
        WS_EX_TOPMOST | WS_EX_LAYERED | WS_EX_NOACTIVATE,
        wc.lpszClassName, L"ZeroElite_Stage",
        WS_POPUP,
    ...
    // Use SetLayeredWindowAttributes only after the window is created to make the black color transparent for the game.
    // This ensures accuracy and prevents anti-cheat detection by providing a "stage" for drawing.
...
{
    // Auto-Pilot Start: Ensure the Black Overlay and Hardware Check start Automatically the moment I run the .exe.
    // Run professional initialization checks (Hardware Check)
